...
const TestimonialsSection = ({ title, subtitle, testimonials }: TestimonialsSectionProps) => {
  return (
    <section className="section-padding bg-brand-coral relative overflow-hidden">
      {/* Decorative background elements */}
      <div className="absolute top-0 left-1/4 w-80 h-80 bg-brand-azure10 rounded-full -translate-y-1/2"></div>
      <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-brand-regentGray10 rounded-full translate-y-1/2"></div>
      <div className="absolute top-1/2 right-0 w-64 h-64 bg-brand-azureDark5 rounded-full translate-x-1/3"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            <div key={index} className="card p-6 relative group overflow-hidden">
